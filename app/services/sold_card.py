...
async def render_sold_card(
    ...
    # "SOLD" big
    font_sold = _get_font(40, bold=True)
    draw.text((bar_x + 16, bar_y + 10), "SOLD", fill=(*_GOLD, 255), font=font_sold)

    # "for" + price big
    font_for = _get_font(22, bold=False)
    font_price = _get_font(42, bold=True)
    price_num = f"Rs.{final_bid:.2f} Cr"
    ...
    ry = bar_y + 10
    draw.text((rx, ry + 8), "for ", fill=(*_LIGHT_BLUE, 180), font=font_for)
    ...
    text_area_h = 90  # space for name + role
    photo_size = min(half_w - 16, content_h - text_area_h, 300)
    ...
    overseas = " [OV]" if player.is_overseas else ""
    name_str = player.name + overseas
    font_name = _get_font(30, bold=True)
    font_role = _get_font(22)

    name_w = _text_width(draw, name_str, font_name)
    name_x = left_x + (half_w - name_w) // 2
    name_y = photo_y + photo_size + 6
    draw.text((name_x, name_y), name_str, fill=(*_GOLD, 255), font=font_name)

    role_w = _text_width(draw, player.role, font_role)
    role_x = left_x + (half_w - role_w) // 2
    draw.text((role_x, name_y + 32), player.role, fill=(*_LIGHT_BLUE, 200), font=font_role)
    ...
    font_team = _get_font(30, bold=True)
    font_code = _get_font(22)

    team_w = _text_width(draw, team.name, font_team)
    team_x = right_x + (half_w - team_w) // 2
    team_y = logo_y + logo_size + 6
    draw.text((team_x, team_y), team.name, fill=(*_GOLD, 255), font=font_team)

    code_w = _text_width(draw, team.short_code, font_code)
    code_x = right_x + (half_w - code_w) // 2
    draw.text((code_x, team_y + 32), team.short_code, fill=(*_LIGHT_BLUE, 200), font=font_code)
...
async def render_unsold_card(
    ...
    draw.text(((UNSOLD_W - lw) // 2, bar_y + 10), "Unsold", fill=(*_GOLD, 255), font=font_label)

    # Player photo — smaller (75% of card width)
    content_y = bar_y + bar_h + 14
    photo_size = int((UNSOLD_W - 2 * (PAD - 2)) * 0.75)
    ...
    label_y = photo_y + photo_size + 10
    font_name = _get_font(26, bold=True)
    font_role = _get_font(20)
    overseas = " [OV]" if player.is_overseas else ""
    name_str = player.name + overseas

    name_w = _text_width(draw, name_str, font_name)
    draw.text(((UNSOLD_W - name_w) // 2, label_y), name_str, fill=(*_GOLD, 255), font=font_name)
