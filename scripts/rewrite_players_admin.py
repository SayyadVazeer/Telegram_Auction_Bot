"""Rewrite players_admin.py with clean formatting."""
import os
...
async def save_player_file_id(message: Message) -> None:
    ...
    async with AsyncSessionLocal() as session:
        ...
        if player is None:
            await message.answer(f"No player found with ID: {player_id_text}")
            return
        player.telegram_file_id = photo.file_id
        await session.commit()
    await message.answer(f"Saved file_id for {player.name} ({player.player_id})")
...
async def swap_player_start(message: Message, state: FSMContext) -> None:
    ...
    if not players:
        await message.answer("You have no players to swap.")
        return
    await state.clear()
...
async def swap_select_from_player(message: Message, state: FSMContext) -> None:
    ...
    if not other_teams:
        await state.clear()
        await message.answer("No other teams to swap with.")
        return
...
async def swap_confirm(message: Message, state: FSMContext) -> None:
    ...
    async with AsyncSessionLocal() as session:
        ...
        if ar1 and ar2:
            ...
            await message.answer("Swap failed: player not found in auction results.")
    await state.clear()
